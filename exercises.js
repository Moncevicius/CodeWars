...
function betterThanAverage(classPoints, yourPoints) {
    return classPoints.reduce((sum, num) => { return sum + num}) / classPoints.length < yourPoints;
}
...
function squareSum(numbers){
    ...
    return numbers.reduce((total, num) => total  + (num * num), initialValue);
}
...
function digitize(n) {
    //code here
    return Array.from(String(n)).reverse().map(v => Number(v));
}
